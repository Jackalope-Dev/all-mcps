...
export function ConfigValidatorTool() {
  ...
  const [formatOverride, setFormatOverride] = useState<ClientFormat | 'auto'>(
    'auto',
  );
  ...
  const { findings, parseError, effectiveFormat } = useMemo(() => {
    if (!raw.trim()) {
      return {
        ...
        effectiveFormat: 'claude' as ClientFormat,
      };
    }
    try {
      const parsed = JSON.parse(raw);
      const format =
        formatOverride === 'auto' ? detectFormat(parsed) : formatOverride;
      trackFeatureUse('config_validator', { format });
      return {
        findings: validateConfig(parsed, format),
        parseError: null as string | null,
      ...
      {raw.trim() && (
        <div>
          {parseError ? (
            ...
            <div
              className="surface-muted"
              style={{ padding: '1rem', borderLeft: '3px solid #10b981' }}
            >
              ✓ Looks good &mdash; valid {CLIENT_FORMAT_LABELS[effectiveFormat]}{' '}
              config.
            </div>
          ) : (
            <div
            ...
            >
              {errors.map((f, i) => (
                ...
                >
                  <Badge variant="default" style={{ marginRight: '0.5rem' }}>
                    error
                  </Badge>
