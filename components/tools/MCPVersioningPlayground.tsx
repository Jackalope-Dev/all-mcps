    ...
    return JSON.stringify(
      {
        ...
        result: {
          ...
          content: [
            {
              type: 'text',
              text: 'Skipped filing the issue since no username was provided.',
            },
          ],
          isError: false,
        },
      ...
      result: {
        ...
        inputRequests: {
          github_login: {
            ...
            params: {
              message:
                'Still need a GitHub username to file this issue. Try again?',
              requestedSchema: {
                type: 'object',
                properties: { name: { type: 'string' } },
                required: ['name'],
    ...
    <Card style={{ padding: '1.5rem' }}>
        ...
        {MRTR_STEPS.map((s, idx) => (
          <button
            ...
            style={{
              ...
              background:
                idx === stepIndex ? 'rgba(0, 229, 255, 0.12)' : 'transparent',
              color:
                idx === stepIndex
                  ? 'var(--accent-color)'
                  : 'var(--text-secondary)',
      ...
      {stepKey === 'initial' && (
        <div>
          ...
          >
            The client calls a tool. It has no username to pass yet, so the
            request only carries what it already knows.
          </p>
      ...
      {stepKey === 'input_required' && (
        <div>
          ...
          >
            The server can&rsquo;t finish without a username. Instead of sending
            its own JSON-RPC request back to the client (the old, now-removed
            pattern), it answers the original request with{' '}
            <code>resultType: &quot;input_required&quot;</code> and an opaque{' '}
            <code>requestState</code> blob the client must echo back unmodified.
          </p>
      ...
      {stepKey === 'user_input' && (
        <div>
          ...
          >
            The client renders the requested schema as a form and asks the user.
            Try each outcome below, an elicitation can be accepted, declined, or
            cancelled, and the protocol defines a distinct response shape for
            each.
          </p>
          <div
            style={{
      ...
      {stepKey === 'retry' && (
        <div>
          ...
          >
            The client retries the <em>original</em> request under a brand-new
            JSON-RPC <code>id</code>, this time carrying{' '}
            <code>inputResponses</code> for the answer and the exact{' '}
            <code>requestState</code> value the server handed back. The client
      ...
      >
        ...
        <Button
          ...
          size="sm"
          onClick={() =>
            setStepIndex((i) => Math.min(MRTR_STEPS.length - 1, i + 1))
          }
...
const OUTCOMES: Record<Era, Record<Era, { works: boolean; text: string }>> = {
  modern: {
    modern: {
      works: true,
      text: 'Works. Calling server/discover first is optional here, since a version mismatch simply comes back as UnsupportedProtocolVersionError and the client retries with a mutually supported version.',
    },
    legacy: {
      works: false,
      text: 'Fails by default. A legacy server has no concept of the modern _meta protocol fields. It may reject the request with an implementation-defined error, stay silent, or even try to process it under legacy rules. On stdio, send server/discover first so an unrecognized-method response lets the client fail deterministically instead of hanging.',
    },
    dual: {
      works: true,
      text: 'Works. A dual-era server decides its behavior from how the request arrives: since this request carries modern _meta fields, the server serves it under the current stateless revision, the same as a modern-only server would.',
    },
  },
  legacy: {
    modern: {
      works: false,
      text: 'Fails, with no way to recover. On stdio the server rejects initialize outright since it is no longer a recognized method. On Streamable HTTP the request is missing the required headers and gets a 400. Legacy clients have no fall-forward mechanism, so the only real fix is upgrading the client.',
    },
    legacy: {
      works: true,
      text: 'Works exactly as it always has. The initialize / initialized handshake negotiates a shared legacy protocol version and the session proceeds as before. Neither side ever speaks the modern, stateless dialect.',
    },
    dual: {
      works: true,
      text: "Works. The server recognizes the client's initialize request as the start of a legacy session and serves it under whichever legacy revision they negotiate. From the client's side, nothing looks different.",
    },
  },
  dual: {
    modern: {
      works: true,
      text: 'Works, and the client stays modern. On stdio its server/discover probe gets back a real DiscoverResult; on HTTP its first modern-shaped request either succeeds or returns a modern error it can act on. It never needs to fall back.',
    },
    legacy: {
      works: true,
      text: 'Works, by falling back. On stdio the probe times out or returns an error that is not a recognized modern one, so the client falls back to initialize. On Streamable HTTP, a modern-shaped request gets a 4xx whose body is not a recognized modern error, so the client falls back to initialize (and, as a last resort, to the deprecated HTTP+SSE transport).',
    },
    dual: {
      works: true,
      text: 'Works either way the client lands. Its probe or first request will read as modern to the server, so it gets served the current stateless revision. Had it opened with initialize instead, the same server would have negotiated a legacy session just as happily. Staying modern is preferable, since it gets the newer error handling and discovery.',
    },
  },
};
        ...
        <div>
          <div
            style={{
              ...
              color: outcome.works ? '#10b981' : '#ef4444',
            }}
          >
