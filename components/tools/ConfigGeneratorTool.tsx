...
import {
  isRemoteHint,
  parseInstallHint,
} from '../../lib/tools/parseInstallHint';
...
export function ConfigGeneratorTool() {
  ...
  function addFromDirectory(server: DirectoryServerHit) {
    trackFeatureUse('config_generator', {
      action: 'add_server',
      server_id: server.id,
    });
    ...
    if (hint && isRemoteHint(hint)) {
      setRows((r) => [
        ...r,
        newRow({ name, url: hint.url, sourceServerId: server.id }),
      ]);
    } else if (hint) {
      setRows((r) => [
        ...r,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              ...
              <Card key={row.id} style={{ padding: '1.25rem' }}>
                ...
                <Input
                  ...
                  value={Object.entries(row.env || {})
                    ...
                    .join(', ')}
                  onChange={(e) =>
                    updateRow(row.id, { env: parseEnvInput(e.target.value) })
                  }
                />
              </Card>
            ))}
      ...
      <section>
        ...
        <CopyBlock
          code={configJson}
          title={CLIENT_CONFIG_PATHS[format][0]}
          language="json"
        />
      </section>
    </div>
