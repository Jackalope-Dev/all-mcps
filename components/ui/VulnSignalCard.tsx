...
export function VulnSignalCard({ server }: { server: Server }) {
  ...
  const osvListUrl = ecosystem && packageName
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.6rem 1.1rem', fontSize: '0.8rem' }}>
        <span style={{ color: critical > 0 ? '#6366f1' : 'var(--text-secondary)', fontWeight: critical > 0 ? 700 : 400 }}>
        ...
        <span style={{ color: 'var(--text-secondary)' }}>
          Medium <strong style={{ color: 'var(--text-primary)' }}>{medium}</strong>
        </span>
        <span style={{ color: 'var(--text-secondary)' }}>
      ...
      <p style={{ margin: 0, fontSize: '0.72rem', color: 'var(--text-secondary)', paddingTop: '0.5rem', borderTop: '1px solid var(--border-color)' }}>
        Scanned {relativeTime} via{' '}
        {osvListUrl ? (
          <a href={osvListUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent-color)' }}>
            OSV.dev
          </a>
        ) : (
          'OSV.dev'
