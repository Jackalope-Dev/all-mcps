...
export function DirectoryBadgeCard({ serverId, serverName }: DirectoryBadgeCardProps) {
  const [style, setStyle] = useState<'directory' | 'featured' | 'shield'>('directory');
  const [copiedKey, setCopiedKey] = useState<string | null>(null);

  const origin = typeof window !== 'undefined' ? window.location.origin : 'https://allmcps.com';
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.55rem' }}>
          <ShieldCheck size={20} style={{ color: 'var(--accent-color)', flexShrink: 0 }} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 600 }}>Badge Style:</span>
        {(
          [
          ...
          <button
            ...
            style={{
              ...
              borderRadius: '6px',
              border: style === opt.id ? '1px solid var(--accent-color)' : '1px solid var(--border-color)',
              background: style === opt.id ? 'rgba(0, 229, 255, 0.12)' : 'var(--bg-muted)',
              color: style === opt.id ? 'var(--accent-color)' : 'var(--text-secondary)',
      ...
      >
        ...
        <img
          ...
          style={{ maxHeight: style === 'directory' ? '40px' : style === 'featured' ? '32px' : '20px' }}
        />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '0.85rem' }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.35rem' }}>
            <span style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-primary)' }}>Markdown (GitHub README)</span>
            <button
              type="button"
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.35rem' }}>
            <span style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-primary)' }}>HTML Embed</span>
            <button
              type="button"
