...
const SITUATIONS = [
  { label: 'Query SQL & Databases', icon: Database, query: 'postgres mysql sqlite database', category: 'Database' },
  { label: 'Web Scraping & Search', icon: Globe, query: 'browser scrape search brave playwright', category: 'Web Search' },
  { label: 'Git & Code Management', icon: Code2, query: 'github git gitlab repository', category: 'Developer Tools' },
  { label: 'Agent Memory & Context', icon: Brain, query: 'memory vector embeddings knowledge', category: 'Productivity' },
  { label: 'Cloud & Infrastructure', icon: Cloud, query: 'aws kubernetes docker cloudflare', category: 'Cloud Platforms' },
  { label: 'Document & PDF Parsing', icon: FileText, query: 'pdf markdown document excel word', category: 'Developer Tools' },
];

export function HeroSection({ totalCount, onSelectSituation }: HeroSectionProps) {
        ...
        <div className="hero-goal-body">
          <div className="hero-goal-header-row">
            ...
            <div className="hero-goal-action-btn-wrap">
              {activeGoal === 'discover' && (
                <a href="#directory-search" className="btn btn-primary btn-sm">
              ...
              <a
                ...
                onClick={(e) => {
                  if (onSelectSituation) {
                    e.preventDefault();
                    onSelectSituation(sit.query, sit.category);
