...
export function PurchaseTracker() {
  ...
  useEffect(() => {
    ...
    const serverId =
      searchParams.get('id') || searchParams.get('serverId') || 'unknown';

    if (
      paidParam &&
      (paidParam === 'priority_review' ||
        paidParam === 'priority' ||
        paidParam === 'featured_7d' ||
        paidParam === 'premium_monthly')
    ) {
      const normalizedSku: PaidSku =
        paidParam === 'priority' ? 'priority_review' : (paidParam as PaidSku);
