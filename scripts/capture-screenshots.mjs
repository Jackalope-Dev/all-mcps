...
console.log(
  'Capturing screenshots with a 2.5s delay to let all animations settle...',
);
...
PAGES.forEach(({ name, url }) => {
  ...
  try {
    execSync(
      `npx playwright screenshot --wait-for-timeout=2500 "${url}" "${brandPath}" --viewport-size=1920,1080`,
      { stdio: 'inherit' },
    );
