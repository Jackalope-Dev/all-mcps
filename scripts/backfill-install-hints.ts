...
function main() {
  ...
  for (const row of affected) {
    ...
    // toCachedInstallFields also returns null for a resolve that must not be cached
    // (a "remote" URL that isn't endpoint-shaped) — same handling as a guess: clear.
    const fields = shouldPersist ? toCachedInstallFields(resolved) : null;
    if (fields) {
      statements.push(
