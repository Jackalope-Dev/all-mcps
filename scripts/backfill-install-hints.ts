...
import { resolveInstallConfig, toCachedInstallFields } from '../lib/installConfig';
...
function main() {
  ...
  console.log(`  ${affected.length} rows affected by the pip/trailing-punctuation bugs`);
  ...
  for (const row of affected) {
    ...
    const shouldPersist = !(resolved.confidence === 'low' && resolved.source === 'heuristic');
  ...
  const outPath = path.join(process.cwd(), 'drizzle', 'backfill-2026-08-07-install-hints.sql');
  const header =
  ...
  console.log(`  ${persisted} corrected, ${cleared} cleared (no better-than-low-confidence guess)`);
