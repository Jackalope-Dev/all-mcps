...
const screenshot2Path = path.resolve('brand-assets/browse-directory-desktop.png');
const screenshot3Path = path.resolve('brand-assets/config-generator-tool-desktop.png');
...
async function runSuperPageAgent(page, targetName, forceRefill = false) {
  ...
  try {
    ...
    const screenshotsArray = [screenshot1Path, screenshot2Path, screenshot3Path, screenshot4Path].filter(p => fs.existsSync(p));
    ...
    for (let i = 0; i < fileInputs.length; i++) {
      ...
      if (info.text.includes('logo') || info.text.includes('icon') || info.text.includes('avatar')) {
        if (fs.existsSync(logoPath)) {
          await input.setInputFiles(logoPath);
          console.log(`  📸 Uploaded LOGO: logo-icon.png`);
          await randomDelay(300, 600);
        }
      } else if (info.text.includes('screenshot') || info.text.includes('preview') || info.text.includes('cover') || info.text.includes('image') || info.text.includes('media') || info.text.includes('gallery')) {
        if (info.multiple && screenshotsArray.length > 0) {
          ...
          console.log(`  🖼️ Uploaded SCREENSHOT #2: browse-directory-desktop.png`);
          await randomDelay(300, 500);
        } else if (i === 3 && screenshotsArray[2]) {
          await input.setInputFiles(screenshotsArray[2]);
          console.log(`  🖼️ Uploaded SCREENSHOT #3: config-generator-tool-desktop.png`);
          await randomDelay(300, 500);
        }
      } else {
  ...
  try {
    ...
    for (const sel of selects) {
      ...
      const text = await page.evaluate(el => `${el.name} ${el.id} ${el.labels?.[0]?.textContent || ''} ${el.parentElement?.textContent || ''}`.toLowerCase(), sel);

      if (text.includes('cat') || text.includes('topic') || text.includes('industry') || text.includes('niche')) {
        const options = await sel.$$('option');
        for (const opt of options) {
          const optText = (await opt.textContent()).toLowerCase();
          if (optText.includes('developer') || optText.includes('ai') || optText.includes('saas') || optText.includes('productivity') || optText.includes('tech')) {
            const val = await opt.getAttribute('value');
            await sel.selectOption(val);
            console.log(`  🏷️ Selected Category: ${(await opt.textContent()).trim()}`);
            await randomDelay(300, 500);
            break;
          }
        }
      } else if (text.includes('price') || text.includes('pricing') || text.includes('model') || text.includes('cost')) {
        const options = await sel.$$('option');
        for (const opt of options) {
          const optText = (await opt.textContent()).toLowerCase();
          if (optText.includes('free') || optText.includes('open source')) {
            const val = await opt.getAttribute('value');
            await sel.selectOption(val);
            console.log(`  💲 Selected Pricing: ${(await opt.textContent()).trim()}`);
            await randomDelay(300, 500);
            break;
          }
        }
  ...
  const inputElements = await page.$$('input:not([type="hidden"]):not([type="submit"]):not([type="file"]):not([type="checkbox"]):not([type="radio"]), textarea, [contenteditable="true"]');
  ...
  for (const el of inputElements) {
    ...
    const isCurrentValueEmpty = await el.evaluate(input => !input.value || input.value.trim() === '');
    ...
    const info = await page.evaluate(field => {
      ...
      const parentText = (field.parentElement?.textContent || '').toLowerCase().slice(0, 150);
      const tag = field.tagName;
      const currentValue = field.value || '';
      return { combined: `${name} ${id} ${placeholder} ${ariaLabel} ${labelText} ${parentText}`, tag, name, id, currentValue };
    }, el);

    const { combined, tag, name, id, currentValue } = info;

    // Skip search bars & video fields
    if (name === 'q' || info.combined.includes('search bar') || id.includes('search_bar')) continue;
    if (combined.includes('video') || combined.includes('youtube') || combined.includes('vimeo') || combined.includes('demo_url')) continue;

    let targetValue = null;
    let labelName = '';

    // 1. Long Description / About / Full Pitch
    if (combined.includes('long_desc') || combined.includes('full_desc') || combined.includes('about') || combined.includes('overview') || combined.includes('details') || (tag === 'TEXTAREA' && (combined.includes('desc') || combined.includes('body')) && !combined.includes('short') && !combined.includes('comment') && !combined.includes('intro'))) {
    ...
    else if (combined.includes('comment') || combined.includes('intro') || combined.includes('maker_note') || combined.includes('story') || combined.includes('founder') || combined.includes('note_to')) {
      targetValue = submissionData.makerComment;
      labelName = 'First Comment / Intro';
    }
    // 4. Tagline / Headline / One-liner
    else if (combined.includes('tagline') || combined.includes('headline') || combined.includes('one_liner') || (combined.includes('pitch') && !combined.includes('comment') && !combined.includes('search'))) {
      targetValue = submissionData.tagline;
      labelName = 'Tagline/Pitch';
    }
    // 5. Search Query Pitch ("What do people search for?")
    else if (combined.includes('search for') || combined.includes('find a tool') || combined.includes('search_query') || combined.includes('search_term') || combined.includes('how_to_find')) {
      targetValue = submissionData.searchQueryPitch;
      labelName = 'Search Queries Pitch';
    }
    // 6. Keywords / Tags
    else if (combined.includes('tag') || combined.includes('keyword') || combined.includes('category_tags') || combined.includes('topics')) {
      targetValue = submissionData.keywords.join(', ');
      labelName = 'Category Tags/Keywords';
    }
    // 7. Twitter Handle / Username vs Twitter URL
    else if (combined.includes('twitter') || combined.includes('x.com') || combined.includes('x_handle') || combined.includes('twitter_handle')) {
      if (combined.includes('user') || combined.includes('handle') || info.combined.includes('@') || name.includes('handle') || id.includes('handle')) {
    ...
    else if (combined.includes('maker_name') || combined.includes('author') || combined.includes('founder_name') || combined.includes('your_name')) {
      targetValue = submissionData.makerName;
      labelName = 'Maker Name';
    }
    // 10. Website URL
    else if (combined.includes('url') || combined.includes('website') || combined.includes('domain') || combined.includes('link')) {
      targetValue = submissionData.url;
      labelName = 'Website URL';
    }
    // 11. Product Name / App Title
    else if (combined.includes('product_name') || combined.includes('app_name') || combined.includes('startup_name') || combined.includes('tool_name') || (combined.includes('title') && !combined.includes('job')) || (name === 'name' || id === 'name')) {
    ...
    if (targetValue) {
      ...
      try {
        await el.pressSequentially(targetValue, { delay: Math.floor(Math.random() * 30) + 25 });
      } catch (err) {
      ...
      console.log(`  ✨ Typed ${labelName}: ${targetValue.length > 40 ? targetValue.slice(0, 40) + '...' : targetValue}`);
...
async function runLLMDirectorySubmitter() {
  ...
  console.log('  ⌨️ Humanized typing simulation (25-55ms randomized keystrokes)');
  ...
  for (let i = 0; i < TARGET_DIRECTORIES.length; i++) {
    ...
    try {
      ...
      while (!proceed) {
        ...
        console.log(`\n👉 Options:`);
        console.log(`   - Type "f" + [ENTER] to RE-FILL page (e.g. after logging in or opening a modal)`);
        console.log(`   - Press [ENTER] to navigate Chrome to next site: ${nextName}`);
