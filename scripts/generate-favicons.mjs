...
(async () => {
  ...
  await page.setContent(`<!DOCTYPE html><html><body style="margin:0;padding:0;background:transparent;">${tileSvgContent}</body></html>`);
  const tileSvgEl = await page.$('svg');
  await tileSvgEl.evaluate((el) => {
    el.setAttribute('width', '512');
    el.setAttribute('height', '512');
  });
  const tilePngBuffer = await tileSvgEl.screenshot({ type: 'png', omitBackground: true });
  ...
  fs.writeFileSync(path.resolve('brand-assets/favicon-tile.png'), tilePngBuffer);
  ...
  await page.setContent(`<!DOCTYPE html><html><body style="margin:0;padding:0;background:transparent;">${logoSvgContent}</body></html>`);
  const logoSvgEl = await page.$('svg');
  await logoSvgEl.evaluate((el) => {
    el.setAttribute('width', '512');
    el.setAttribute('height', '512');
  });
  const logoPngBuffer = await logoSvgEl.screenshot({ type: 'png', omitBackground: true });
  ...
  console.log('Successfully generated transparent logo PNGs and dark squircle tile favicon PNGs!');
  await browser.close();
})();
