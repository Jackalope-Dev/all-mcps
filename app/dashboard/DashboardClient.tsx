...
export default function DashboardClient({
  ...
  const startEdit = (server: Server) => {
    ...
    setForm({
      ...
      remoteEndpointUrl:
        (p?.remoteEndpointUrl as string) || server.remoteEndpointUrl || '',
      suggestedInstallCommand:
  ...
  const submitEdit = async () => {
    ...
    try {
      const payload = {
        ...
        // Sent even when empty: clearing this is how an owner suppresses the
        // generated install buttons, so it must reach the diff as ''.
        remoteEndpointUrl: form.remoteEndpointUrl,
            ...
            >
              ...
              {activeTab === 'edit' && (
                <div className="dashboard-edit-form">
                  ...
                  <div>
                    <label style={fieldLabelStyle}>Remote MCP endpoint</label>
                    <input
                      className="form-input"
                      type="url"
                      value={form.remoteEndpointUrl}
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          remoteEndpointUrl: e.target.value,
                        }))
                      }
                      placeholder="https://example.com/mcp"
                    />
                    <p className="submit-hint" style={{ marginTop: '0.35rem' }}>
                      The URL clients connect to. Your listing&rsquo;s one-click
                      install buttons are generated from this — leave it empty
                      to remove them until it&rsquo;s right.
                    </p>
                  </div>
