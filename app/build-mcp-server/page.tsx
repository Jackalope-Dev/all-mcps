...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    },
    {
      ...
      item: 'https://allmcps.com/build-mcp-server',
    },
  ],
};
          ...
          <div className="surface page-panel min-w-0">
            ...
            >
              ...
              <p className="text-lead" style={{ margin: 0 }}>
                A complete, hands-on developer guide to building, testing,
                deploying, and publishing custom Model Context Protocol servers
                in TypeScript and Python. New to MCP itself? Start with{' '}
            ...
            <div className="markdown-body">
              ...
              <p>
                An <strong>MCP Server</strong> is a lightweight process that
                exposes capabilities &mdash; tools, data resources, and prompt
                templates &mdash; over standard JSON-RPC 2.0. Any MCP-compatible
                AI client (Claude Desktop, Claude Code, Cursor, and others) can
                connect to your server and use those capabilities on demand,
                without you writing custom integration code for each client.
              </p>

              <h2 id="choosing-stack">Choosing Your Stack</h2>
              <p>
                MCP has officially maintained SDKs in several languages.
                TypeScript and Python are the most mature, with the largest
                ecosystem of example servers to learn from &mdash; this guide
                covers both in full. If your project already lives in another
                language, these SDKs work the same way conceptually:
              </p>
              <ul>
                <li>
              ...
              <p>Install the official SDK and a schema validator:</p>
              <CopyBlock code={`npm install @modelcontextprotocol/sdk zod`} />
              <p>
                Here is a complete working TypeScript server that exposes a{' '}
                <strong>tool</strong>, a <strong>resource</strong>, and a{' '}
...
main().catch(console.error);`}
              ...
              <p>
                Python developers can use the official <code>mcp</code>{' '}
                library&rsquo;s FastMCP wrapper, which turns plain decorated
                functions into MCP primitives:
              </p>
              <CopyBlock code={`pip install "mcp[cli]"`} />
              <p>
                The same tool, resource, and prompt as above, in a few lines of
                Python:
              </p>
              <CopyBlock
...
if __name__ == "__main__":
    mcp.run()`}
              ...
              <ul>
                <li>
                  <strong>Tools</strong>: Functions that execute side effects or
                  perform calculations. Always provide clear parameters and a
                  descriptive JSON schema so the AI model knows when and how to
                  call them &mdash; see <code>calculate_sum</code> /{' '}
                  <code>add_numbers</code> above.
                </li>
                <li>
                  <strong>Resources</strong>: Read-only data sources identified
                  by URIs (e.g. <code>config://app</code>,{' '}
                  <code>file:///logs/app.log</code>, or{' '}
                  <code>db://users/123</code>). Clients can list and read them
                  without invoking a tool.
                </li>
                <li>
                  <strong>Prompts</strong>: Reusable template workflows, like{' '}
                  <code>summarize</code> above, that users can invoke directly
                  inside their AI client&rsquo;s interface.
                </li>
              </ul>

              <h2 id="testing">Testing Locally</h2>
              <p>
                Test your server directly in your browser without wiring it into
                a full AI client, using the official MCP Inspector:
              </p>
              <CopyBlock
                code={`npx @modelcontextprotocol/inspector node dist/index.js`}
              />
              <p>
                This launches an interactive UI where you can call tools, read
                resources, and get prompts, while watching the raw JSON-RPC
  ...
  "mcpServers": {
    "my-mcp-server": {
      ...
      "args": ["/absolute/path/to/dist/index.js"]
              ...
              />
              <p>
                For a Python server, swap the command for{' '}
                <code>
                  uv run --directory /absolute/path/to/project server.py
                </code>
                . Fully restart the client (quit and reopen) after editing its
                config &mdash; see the{' '}
                <Link href="/guide">LLM Agents Guide</Link> for the exact config
                file locations. Claude Code can add a local server directly from
                the command line instead:
              </p>
              <CopyBlock
                code={`claude mcp add my-mcp-server -- node /absolute/path/to/dist/index.js`}
              />

              <h2 id="deploying">Deploying a Remote Server</h2>
              <p>
                Everything above uses the <strong>stdio transport</strong>: your
                AI client launches the server as a local subprocess. For a
                server that multiple people share, or that needs to run
                somewhere other than the user&rsquo;s machine, expose it over
                HTTP/SSE instead as a <strong>remote server</strong>. One common
                way to host a remote MCP server is on Cloudflare Workers using
                the <code>agents</code> package&rsquo;s <code>McpAgent</code>{' '}
                class:
              </p>
              <CopyBlock
  ...
  fetch(request: Request, env: unknown, ctx: ExecutionContext) {
    ...
    return new Response("Not found", { status: 404 });
              ...
              <p>
                ...
                for authentication, session handling, and other remote-server
                details.
              </p>
              ...
              <ol>
                <li>
                  Publish your package to npm (Node.js) or PyPI (Python), or
                  host it on GitHub with a tagged release.
                </li>
                <li>
                  Write a <code>README.md</code> with a clear{' '}
                  <code>claude_desktop_config.json</code> snippet and a list of
                  any required environment variables or API keys.
                </li>
                <li>
                  Pick an OSS license &mdash; MIT and Apache-2.0 are the most
                  widely accepted for MCP servers.
                </li>
                <li>
                  Tag a semantic-versioned release so users can pin a specific
                  version.
                </li>
                <li>
                  Head over to our <Link href="/submit">Submit Page</Link> to
                  list your server on AllMCPs and reach thousands of AI
                  developers.
                </li>
              </ol>
                ...
                <FaqSection
                  ...
                  items={[
                    ...
                    },
                    {
                      ...
                      answer:
                        'Function calling is a model feature for invoking a single function schema. MCP is a standardized client-server protocol: one MCP server can expose many tools, resources, and prompts, and any MCP-compatible client can connect to it seamlessly.',
                    },
                    {
                      question:
                    ...
                    },
                    {
                      ...
                      answer:
                        "A local stdio server inherits the permissions of the user running it, so it typically doesn't need its own auth layer. A remote HTTP server should require a bearer token or similar credential.",
                    },
                    {
                      question: 'How do I get my MCP server listed on AllMCPs?',
                      answer: (
                        <span>
                          Publish it to npm, PyPI, or GitHub with a clear README
                          and setup instructions, then submit it through the{' '}
                          <Link href="/submit">AllMCPs submission form</Link>{' '}
                          for review.
                        </span>
                      ),
                    },
              ...
              <ul>
                ...
                <li>
                  Connecting an existing server to a client instead of building
                  one? See the <Link href="/guide">LLM Agents Guide</Link>.
                </li>
                <li>
                  Ready to take your server to production? Read our{' '}
                  <Link href="/deploy-mcp-server">
                    Deploying &amp; Hosting Remote MCP Servers Guide
                  </Link>
                  .
                </li>
                <li>
                  Ready to find inspiration?{' '}
                  <Link href="/browse">Browse the directory</Link> or explore by{' '}
