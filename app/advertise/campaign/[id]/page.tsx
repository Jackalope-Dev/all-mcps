...
import { calculateCtr, formatUsdAmount } from '../../../../lib/ads';
...
async function getCampaignData(id: string) {
  try {
    ...
    const placementMap: Record<
      string,
      { impressions: number; clicks: number; visitors: Set<string> }
    > = {};
    ...
    const placementStats: PlacementBreakdown[] = Object.entries(
      placementMap,
    ).map(([placement, stats]) => ({
      placement,
    ...
    const uniqueReach =
      allSessions.size > 0
        ? allSessions.size
        : Math.round(ad.impressionsServed * 0.78);
...
export default async function CampaignDashboardPage({
    ...
    return (
      <PageShell variant="content" panel>
        <PageHeader
          ...
          description="The requested ad campaign could not be found."
        />
  ...
  const isSubmittedNotice =
    (search.submitted === '1' || search.payment === 'success') &&
    !awaitingPayment;
  ...
  const progressPct = Math.min(
    ...
    Number(
      (
        (ad.impressionsServed / Math.max(1, ad.totalImpressionsPurchased)) *
        100
      ).toFixed(1),
    ),
  );
  ...
  const effectiveCpc =
    ad.clicksCount > 0
      ? (ad.amountPaidCents / 100 / ad.clicksCount).toFixed(2)
      : null;
  const deliveredCpm =
  ...
  const maxPlacementImpressions = Math.max(
    ...placementStats.map((p) => p.impressions),
    1,
  );
  ...
  const campaignEndMs = ad.completedAt
    ? new Date(ad.completedAt).getTime()
    : Date.now();
  const campaignDurationDays = Math.max(
    1,
    Math.round((campaignEndMs - campaignStartMs) / (1000 * 60 * 60 * 24)),
  );

  // Ad frequency: avg impressions per unique developer
  const adFrequency =
    uniqueReach > 0 ? (ad.impressionsServed / uniqueReach).toFixed(1) : null;

  // Cost per unique developer reached
  const costPerDev =
    uniqueReach > 0
      ? (ad.amountPaidCents / 100 / uniqueReach).toFixed(2)
      : null;
    ...
    <PageShell variant="content" panel>
      <PageHeader
        ...
        description={
          <>
            ...
            . All impressions counted via MRC-compliant 50%+ in-viewport
            detection.
          </>
        }
      />
      ...
      {awaitingPayment && (
        ...
        >
          <DollarSign
            size={24}
            style={{ color: 'var(--status-pending)', flexShrink: 0 }}
          />
          <div style={{ flex: 1, minWidth: '220px' }}>
            ...
            >
              This campaign hasn&apos;t been paid for yet, so it won&apos;t
              enter the moderation queue until checkout is complete. Nothing is
              charged or reviewed until then.
            </div>
          </div>
          <a
        ...
        >
          ...
          <div>
            ...
            <div
              style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}
            >
              Something went wrong setting up payment. Please try the button
              above again, or contact{' '}
              <a
                href="mailto:support@allmcps.com"
      ...
      {isSubmittedNotice && (
        <div
          style={{
            ...
            borderRadius: '14px',
            background:
              'linear-gradient(135deg, rgba(0, 229, 255, 0.1), rgba(0, 123, 255, 0.1))',
        ...
        >
          ...
          <div>
            ...
            <div
              style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}
            >
              Your campaign is in the moderation queue. Once approved (typically
              within 12–24 hours), impressions will begin serving immediately
              across all placements and AI feeds. Bookmark this URL to monitor
        ...
        >
          ...
          <div>
            ...
            <div
              style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}
            >
              Reason: {ad.rejectionReason}. Your payment has been automatically
              refunded in full via Stripe.
            </div>
          </div>
        </div>
      )}
      ...
      >
        ...
        <div>
          ...
          >
            {formatUsdAmount(ad.amountPaidCents)}
          </div>
          {ad.stripeInvoiceUrl && (
            <a
        ...
        <div>
          ...
          >
            <CalendarDays size={13} style={{ color: 'var(--accent-color)' }} />{' '}
            {campaignDurationDays} {campaignDurationDays === 1 ? 'day' : 'days'}
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            <Eye size={15} style={{ color: 'var(--accent-color)' }} /> Verified
            Impressions
          </div>
          <div
            style={{
        ...
        >
          ...
          >
            Developer tools avg:{' '}
            <strong style={{ color: 'var(--text-primary)' }}>~1.8%</strong>
          </div>
        </div>
        ...
        >
          ...
          >
            <Users size={15} style={{ color: 'var(--accent-color)' }} /> Unique
            Developer Reach
          </div>
          <div
            style={{
      ...
      >
        {deliveredCpm && (
          ...
          >
            ...
            >
              Actual cost per 1k verified views
            </div>
          </div>
        )}
        {effectiveCpc && (
        ...
        {costPerDev && (
          ...
          >
            ...
            >
              <Target size={14} style={{ color: 'var(--accent-color)' }} /> Cost
              / Developer
            </div>
            <div
              style={{
        ...
        {adFrequency && (
          ...
          >
            ...
            >
              <Repeat size={14} style={{ color: 'var(--accent-color)' }} /> Ad
              Frequency
            </div>
            <div
              style={{
            ...
            >
              Avg impressions per unique dev
            </div>
          </div>
        )}
        <div
        ...
        >
          ...
          >
            <Bot size={14} style={{ color: 'var(--accent-color)' }} /> AI Agent
            Injections
          </div>
          <div
            style={{
          ...
          >
            {aiInjectionCount.toLocaleString()}
          </div>
          <div
            style={{
              fontSize: '0.7rem',
          ...
          >
            Complimentary MCP context injections, free
          </div>
        </div>
      </div>
      ...
      {topPlacement && topPlacement.clicks > 0 && (
        <div
          style={{
            ...
            borderRadius: '14px',
            background:
              'linear-gradient(135deg, rgba(34, 197, 94, 0.06), rgba(0, 229, 255, 0.06))',
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: '200px' }}>
            ...
            >
              {topPlacement.ctr}% CTR · {topPlacement.clicks.toLocaleString()}{' '}
              clicks · {topPlacement.impressions.toLocaleString()} impressions
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          >
            <div>
              ...
              >
                <Layers size={17} style={{ color: 'var(--accent-color)' }} />{' '}
                Multi-Placement Performance
              </h3>
              <p
                style={{
                  margin: '0.25rem 0 0',
              ...
              >
                Real-time delivery breakdown across all automatic placement
                surfaces.
              </p>
            </div>
          </div>
          ...
          >
            {placementStats.map((p) => {
              const share =
                ad.impressionsServed > 0
                  ? (p.impressions / ad.impressionsServed) * 100
                  : 0;
              const widthPct = Math.max(
                2,
                Math.round((p.impressions / maxPlacementImpressions) * 100),
              );
              const seriesColor =
                PLACEMENT_COLOR_VAR[p.placement] || 'var(--chart-series-5)';
              return (
                <div
                  key={p.placement}
                  style={{
          ...
          <div style={{ overflowX: 'auto' }}>
            ...
            >
              <thead>
                ...
                >
                  <th style={{ padding: '0.7rem 0.5rem', fontWeight: 600 }}>
                    Placement
                  </th>
                  <th
                    style={{
              ...
              <tbody>
                {placementStats.map((p) => {
                  const share =
                    ad.impressionsServed > 0
                      ? ((p.impressions / ad.impressionsServed) * 100).toFixed(
                          1,
                        )
                      : '0.0';
                  return (
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: '220px' }}>
              ...
              >
                Your ad has been surfaced directly inside AI agent / MCP
                protocol responses{' '}
                <strong style={{ color: 'var(--text-primary)' }}>
                  {aiInjectionCount.toLocaleString()}{' '}
                  {aiInjectionCount === 1 ? 'time' : 'times'}
                </strong>{' '}
                — a complimentary bonus placement that never draws down your
                purchased impression credits.
              </div>
            </div>
          </div>
        </div>
      ...
      {dailyTimeline.length > 1 && (
        ...
        >
          ...
          >
            ...
            >
              <Activity size={17} style={{ color: 'var(--accent-color)' }} />{' '}
              Daily Delivery Velocity
            </h3>
            <span
              style={{ fontSize: '0.73rem', color: 'var(--text-secondary)' }}
            >
          ...
          >
            {dailyTimeline.map((day) => {
              const heightPct = Math.max(
                8,
                Math.round((day.impressions / maxDailyImp) * 100),
              );
              return (
                <div
                  key={day.date}
          ...
          >
            ...
            >
              ...
              {dailyCtr.map((d, i) => {
                const x =
                  dailyCtr.length > 1 ? (i / (dailyCtr.length - 1)) * 600 : 300;
                const y = 60 - (d.ctr / maxDailyCtr) * 50;
                return (
                  <circle
                    key={d.date}
          ...
          >
            ...
            >
              ...
              <strong style={{ color: 'var(--text-primary)' }}>
                {Math.round(
                  dailyTimeline.reduce((s, d) => s + d.impressions, 0) /
            ...
            >
              ...
              <strong style={{ color: 'var(--text-primary)' }}>
                {Math.round(
                  dailyTimeline.reduce((s, d) => s + d.clicks, 0) /
                    dailyTimeline.length,
                ).toLocaleString()}
              </strong>
            </div>
              ...
              <strong style={{ color: 'var(--text-primary)' }}>
                {
                  dailyTimeline.reduce(
                    (best, d) => (d.impressions > best.impressions ? d : best),
                    dailyTimeline[0],
                  ).date
                }
              </strong>
      ...
      <div
        ...
        style={{
          ...
          marginBottom: '2rem',
          background:
            'linear-gradient(135deg, rgba(0,229,255,0.025), rgba(0,112,243,0.015), var(--bg-surface))',
        }}
      >
        <h4
          style={{
        ...
        >
          <ShieldCheck size={17} style={{ color: 'var(--verified-green)' }} />{' '}
          Transparency &amp; Quality Guarantees
        </h4>
        <div
          style={{
            display: 'grid',
        ...
        >
          <div>
            ...
            >
              Impressions only count when ≥50% of the ad card is physically
              visible in the user&apos;s active viewport via
          ...
          <div>
            ...
            >
              Cryptographic session hashing deduplicates rapid reloads and
              filters automated crawlers from impression counts.
            </div>
          </div>
          <div>
            <div
            ...
            >
              Your campaign runs until every purchased impression credit is
              fulfilled. If rejected, Stripe issues an automatic 100% refund.
            </div>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <SponsorAdUnit
          placement={
            ad.placement === 'all' ? 'directory_inline' : (ad.placement as any)
          }
          previewAd={{
            title: ad.title,
      ...
      <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
        ...
        >
          <RefreshCw size={11} /> Analytics update on each page load. Bookmark
          this URL for anytime access.
        </span>
      </div>
