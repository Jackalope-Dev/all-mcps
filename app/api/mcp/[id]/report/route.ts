...
const REPORT_REASONS = ['broken_install', 'misleading', 'malicious', 'dead_link', 'other'] as const;
    ...
    if (ipHash) {
      ...
      const [hourly, perListingDaily] = await Promise.all([
        db
          .select({ n: count() })
          .from(reports)
          .where(and(eq(reports.reporterIpHash, ipHash), gte(reports.createdAt, new Date(now - HOUR_MS)))),
        db
          .select({ n: count() })
          .from(reports)
          .where(
      ...
      if ((hourly[0]?.n ?? 0) >= MAX_REPORTS_PER_HOUR_GLOBAL) {
        return NextResponse.json(
          { error: 'Too many reports from this connection recently. Please try again later.' },
          { status: 429 }
        );
      }
      if ((perListingDaily[0]?.n ?? 0) >= MAX_REPORTS_PER_DAY_PER_LISTING) {
        return NextResponse.json(
          { error: "You've already reported this listing recently — our team will take a look." },
          { status: 429 }
        );
      }
    }
