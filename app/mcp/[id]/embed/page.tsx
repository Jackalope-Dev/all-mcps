        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
             {(server.isOfficial || server.isPremium) && (
                <span style={{ fontSize: '0.65rem', padding: '0.2rem 0.4rem', borderRadius: '4px', backgroundColor: 'rgba(16, 185, 129, 0.12)', border: '1px solid rgba(16, 185, 129, 0.3)', color: '#047857', fontWeight: 700 }}>Verified</span>
             )}
            <span style={{ fontSize: '0.65rem', padding: '0.2rem 0.4rem', borderRadius: '4px', backgroundColor: 'var(--embed-badge-bg)', border: '1px solid var(--embed-badge-border)', color: 'var(--embed-badge-text)', fontWeight: 600 }}>
              {server.category}
            </span>
          </div>
        </div>
        
        <h3 style={{ fontSize: '1.125rem', margin: org ? '0 0 0.15rem 0' : '0 0 0.5rem 0', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flexShrink: 0, color: 'var(--embed-text)' }}>
          {displayName}
        </h3>
        {org && (
          <div style={{ fontSize: '0.7rem', color: 'var(--embed-subtext)', margin: '0 0 0.5rem 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flexShrink: 0 }}>
            {org}
          </div>
        )}

        <div style={{ fontSize: '0.8rem', margin: '0 0 1rem 0', flexGrow: 1, flexShrink: 0, minHeight: '3.6rem', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden', color: 'var(--embed-subtext)', lineHeight: 1.5 }}>
          <SafeMarkdown content={server.description || 'No description provided.'} isInline />
        </div>
        
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--embed-border)', paddingTop: '0.75rem', flexShrink: 0, minHeight: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--embed-accent)', fontSize: '0.75rem', fontWeight: 600, lineHeight: 1 }}>
             <Terminal size={12} style={{ flexShrink: 0 }} /> Install via AllMCPs
          </div>
          <div style={{ display: 'flex', alignItems: 'center', fontSize: '0.65rem', color: 'var(--embed-footer)', fontWeight: 500, letterSpacing: '0.02em', lineHeight: 1 }}>
            allmcps.com
          </div>
        </div>
      </Link>
