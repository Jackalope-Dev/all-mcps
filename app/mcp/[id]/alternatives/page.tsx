...
export async function generateMetadata({
  ...
  const summaryText = server.aiSummary?.trim() || server.description;
...
/**
 * Runner shown in the comparison table, or null when nothing confirmed one.
 * This used to end in `cfg.command || 'npx'`, so every listing we had no install
 * details for was published in a comparison table as an npx server. A guessed
 * runner is not a comparison point — omit the cell instead.
 */
function getInstallLabel(s: Server): string | null {
  ...
  if (isUnverifiedInstall(cfg)) return null;
...
export default async function AlternativesPage({
  ...
  const topAltNames = alternatives
    .slice(0, 3)
    .map((a) => parseServerName(a.name).displayName);
  const altNamesStr =
    topAltNames.length > 0
      ? topAltNames.join(', ')
      : 'other featured MCP tools';
      ...
      },
      {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 3,
            name: categoryLabel,
            item: `${SITE}/categories/${catSlug}`,
          },
          {
            '@type': 'ListItem',
            position: 4,
            name: displayName,
            item: `${SITE}/mcp/${server.id}`,
          },
      ...
      >
        ...
        <nav aria-label="Breadcrumb" style={{ marginBottom: '2rem' }}>
          <ol className="breadcrumb">
            ...
            <li>
              <Link href={`/categories/${catSlug}`}>{categoryLabel}</Link>
            </li>
            <li className="breadcrumb-separator">
              <ChevronRight size={12} />
            </li>
            <li>
              <Link href={`/mcp/${server.id}`}>{displayName}</Link>
            </li>
            <li className="breadcrumb-separator">
              <ChevronRight size={12} />
            </li>
        ...
        <section style={{ marginBottom: '2.5rem', maxWidth: '820px' }}>
          ...
          >
            ...
            ? Whether you need a different runtime environment, custom
            authentication support, or alternative API integrations in the{' '}
            <strong>{categoryLabel}</strong> ecosystem, we have cataloged and
            compared the top alternative MCP servers below.
          </p>
          ...
          {server.aiUseCases && server.aiUseCases.length > 0 && (
            ...
            >
              ...
              >
                ...
                Primary Use Cases for {displayName}:
              </div>
              <ul
                style={{
                  margin: 0,
        ...
        {alternatives.length > 0 && (
          <section style={{ marginBottom: '3.5rem' }}>
            ...
            >
              ...
              Specification Matrix: {displayName} vs Alternatives
            </h2>
            <p
              style={{
                fontSize: '0.9rem',
            ...
            >
              Direct comparison of key metrics, runtimes, authentication
              methods, and community popularity.
            </p>
              ...
              >
                ...
                <tbody>
                  ...
                  >
                    ...
                    >
                      {server.copies ? server.copies.toLocaleString() : '—'}
                    </td>
                  ...
                  {alternatives.map((alt) => {
                    ...
                    return (
                      ...
                      >
                        <td
                          style={{ padding: '0.85rem 1rem', fontWeight: 700 }}
                        >
                          <Link
                            href={`/mcp/${alt.id}`}
                            style={{
                        ...
                        >
                          {alt.authType
                            ? AUTH_TYPE_LABELS[alt.authType as AuthType] ||
                              alt.authType
                            : 'Free/None'}
                        </td>
                        <td
                        ...
                        >
                          {alt.pricingModel
                            ? PRICING_MODEL_LABELS[
                                alt.pricingModel as PricingModel
                              ] || alt.pricingModel
                            : 'Free'}
                        </td>
                        <td
                        ...
                        >
                          {alt.githubStars
                            ? alt.githubStars.toLocaleString()
                            : '—'}
                        </td>
                        <td
                          style={{
                        ...
                        >
                          {alt.copies ? alt.copies.toLocaleString() : '—'}
                        </td>
                        <td style={{ padding: '0.85rem 1rem' }}>
                          <Link
        ...
        <section style={{ marginBottom: '3.5rem' }}>
          ...
          >
            ...
            Detailed Evaluation of {displayName} Alternatives
          </h2>
          ...
          {alternatives.length > 0 ? (
            ...
            >
              {alternatives.map((alt) => {
                ...
                const altSummary =
                  alt.aiSummary || alt.aiOverview || alt.description;
                const featuresList =
                  alt.aiFeatures && alt.aiFeatures.length > 0
                    ? alt.aiFeatures.slice(0, 3)
                    : null;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <SafeMarkdown
                        content={altSummary || 'No description provided.'}
                        isInline
                      />
                    </p>
            ...
            >
              <p style={{ color: 'var(--text-secondary)', margin: 0 }}>
                We don&rsquo;t have close alternatives to{' '}
                <Link href={`/mcp/${server.id}`}>{displayName}</Link> catalogued
                yet. Explore more {categoryLabel} servers in our directory.
              </p>
            </div>
          )}
        </section>
