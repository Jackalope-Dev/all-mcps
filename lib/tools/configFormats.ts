...
function rowToServerEntry(row: ServerRow): Record<string, unknown> {
  if (row.url) {
    return row.env && Object.keys(row.env).length
      ? { url: row.url, env: row.env }
      : { url: row.url };
  }
  const entry: Record<string, unknown> = { command: row.command || '' };
  if (row.args?.length) entry.args = row.args;
...
export function detectFormat(parsed: unknown): ClientFormat {
  if (
    ...
    'servers' in (parsed as Record<string, unknown>)
  ) {
    return 'vscode';
  }
  return 'claude';
    ...
    return [
      {
        level: 'error',
        path: '$',
        message: `Missing top-level "${key}" key for this client format.`,
      },
    ];
  }
  if (
    ...
    return [
      {
        level: 'error',
        path: `$.${key}`,
        message: `"${key}" must be an object mapping server names to their config.`,
      },
    ];
  }
  ...
  for (const [name, value] of Object.entries(
    ...
    if (!hasCommand && !hasUrl) {
      findings.push({
        level: 'error',
        path,
        message: 'Entry needs a non-empty "command" or "url".',
      });
    }
    if (hasCommand && hasUrl) {
      findings.push({
        ...
        path,
        message:
          'Entry has both "command" and "url" — only one transport is normally used.',
      });
    }
    if ('args' in entry) {
    ...
    if ('env' in entry) {
      ...
      if (typeof env !== 'object' || env === null || Array.isArray(env)) {
        ...
        findings.push({
          level: 'error',
          path: `${path}.env`,
          message: 'All "env" values must be strings.',
        });
      }
    }
  }
