...
/**
 * Convenience wrapper for callers (like the MCP tool dispatcher) that don't
 * already have a drizzle handle — resolves the Cloudflare D1 binding itself
 * and returns null (never throws) if the binding is unavailable.
 */
export async function getAgentAuthDb(): Promise<any | null> {
  try {
...
/**
 * Resolves agent identity from either a standard `Authorization: Bearer`
 * header, or (fallback, for MCP clients that can't set custom headers per
 * tool call) a raw token string passed as a tool argument.
 */
export async function resolveAgentAuthFlexible(
  db: any,
  authorizationHeader: string | null,
