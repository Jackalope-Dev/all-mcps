...
// 5. Test humanizeSlug
assert(humanizeSlug('cueapi-mcp') === 'CueAPI MCP' || humanizeSlug('cueapi-mcp') === 'Cueapi MCP', 'Should title-case slug words');
assert(humanizeSlug('atlaso-labs') === 'Atlaso Labs', 'Should title-case hyphenated slug');
assert(humanizeSlug('some-repo').includes('MCP') === false, 'Should not inject MCP that is not present');
...
assert(extractReadmeTitle(readmeWithTitle) === 'Cool Weather MCP', `Should extract heading, got: ${extractReadmeTitle(readmeWithTitle)}`);
assert(extractReadmeTitle('# mcp-server\n\nSome text') === null, 'Should reject a heading that is itself generic');
assert(extractReadmeTitle(null) === null, 'Should return null for missing readme');
assert(extractReadmeTitle('# Quick Start\n\nRun npm install.') === null, 'Should reject a doc-section heading masquerading as the title');
assert(extractReadmeTitle('# Table of Contents\n- [Install](#install)') === null, 'Should reject Table of Contents heading');
assert(extractReadmeTitle('# What is Shipeasy?\n\nA thing.') === null, 'Should reject a question-style heading');
assert(extractReadmeTitle('# @scope/pkg-mcp\n\nText') === null, 'Should reject a bare space-free package-name heading, preferring slug fallback');
const readmeHtmlH1 = `<p align="center"><img src="logo.png"></p>\n\n<h1 align="center">Weather Wizard MCP</h1>\n\n## Quick Start\n\nnpm install`;
assert(extractReadmeTitle(readmeHtmlH1) === 'Weather Wizard MCP', `Should prefer an HTML h1 over a later markdown section heading, got: ${extractReadmeTitle(readmeHtmlH1)}`);
assert(extractReadmeTitle('# The tools\n\nText') === null, 'Should reject "The tools" (filler first word)');
assert(extractReadmeTitle('# Tools Available\n\nText') === null, 'Should reject "Tools Available" (section-phrase heading)');
assert(extractReadmeTitle('# 1 — The skill (every AI coding agent)\n\nText') === null, 'Should reject a numbered doc-step heading');
assert(extractReadmeTitle('# Skillsforge MCP\n\nText') === 'Skillsforge MCP', 'Should NOT false-positive-reject a real title that merely starts with a blocked word as a substring');
assert(extractReadmeTitle('# Whatsapp MCP\n\nText') === 'Whatsapp MCP', 'Should NOT false-positive-reject "Whatsapp" for containing "what"');
assert(extractReadmeTitle('# Let your agent install it\n\nText') === null, 'Should reject a sentence-case doc instruction masquerading as a title');
assert(extractReadmeTitle('# Formatix AI — MCP Server\n\nText') === 'Formatix AI — MCP Server', 'Should accept a real Title Case heading with an em-dash subtitle');
assert(extractReadmeTitle('# Top 5 Stocks MCP Server\n\nText') === 'Top 5 Stocks MCP Server', 'Should accept Title Case headings containing numbers');
assert(extractReadmeTitle('# Development Status\n\nBeta.') === null, 'Should reject a short heading that is just a section label, even mid-word');
assert(extractReadmeTitle('# Quick Setup\n\nRun npm install.') === null, 'Should reject "Quick Setup" (section noun not at position 0)');
assert(extractReadmeTitle('# SVGator MCP Server — Documentation\n\nText') === null, 'Should reject a real name with a trailing section-noun suffix');

// 7. Test deriveServerName
assert(
  deriveServerName({ currentName: 'mcp', url: 'https://github.com/atlaso-labs/mcp', ghRepo: { owner: 'atlaso-labs', repo: 'mcp' }, readme: null }) === 'Atlaso Labs MCP',
...
assert(
  deriveServerName({ currentName: 'Tavily MCP Server', url: 'https://github.com/tavily/mcp', ghRepo: { owner: 'tavily', repo: 'mcp' }, readme: null }) === null,
  'Should not touch an already-descriptive name'
);
assert(
  deriveServerName({ currentName: 'mcp', url: 'https://eevy.ai/mcp', ghRepo: null, readme: null }) === 'Eevy MCP',
  'Should humanize hostname for non-GitHub listings'
);
assert(
  deriveServerName({ currentName: 'mcp', url: 'https://app.fiuto.ai', ghRepo: null, readme: null }) === 'Fiuto MCP',
