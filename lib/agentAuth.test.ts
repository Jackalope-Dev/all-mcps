...
assert(
  isValidAgentScope('listings:claim'),
  '"listings:claim" should be a valid scope',
);
assert(
  !isValidAgentScope('listings:delete'),
  'Unsupported scope should be rejected',
);
...
assert(
  JSON.stringify(parseAgentScopes(null)) ===
    JSON.stringify(DEFAULT_AGENT_SCOPES),
  'null scopes column should fall back to DEFAULT_AGENT_SCOPES',
);
assert(
  JSON.stringify(parseAgentScopes('not json')) ===
    JSON.stringify(DEFAULT_AGENT_SCOPES),
  'Malformed scopes JSON should fall back to DEFAULT_AGENT_SCOPES',
);
assert(
  JSON.stringify(parseAgentScopes('[]')) ===
    JSON.stringify(DEFAULT_AGENT_SCOPES),
...
assert(
  JSON.stringify(parseAgentScopes(serializeAgentScopes(['listings:claim']))) ===
    JSON.stringify(['listings:claim']),
  'A serialized valid scope set should parse back unchanged',
);
assert(
  JSON.stringify(parseAgentScopes('["listings:claim","bogus:scope"]')) ===
    JSON.stringify(['listings:claim']),
...
assert(
  hasAgentScope({ scopes: ['listings:claim'] }, 'listings:claim'),
  'Agent with the scope should pass the check',
);
assert(
  !hasAgentScope({ scopes: [] }, 'listings:claim'),
  'Agent without the scope should fail the check',
);

// 5. AGENT_SCOPES is the single source of truth referenced by oauth-protected-resource / auth.md.
assert(
  AGENT_SCOPES.includes('listings:claim'),
  'AGENT_SCOPES should include listings:claim',
);
