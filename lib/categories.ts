...
export const CATEGORY_GROUPS: CategoryGroup[] = [
  {
    ...
    lightGradient:
      'linear-gradient(135deg, rgba(2, 132, 199, 0.08) 0%, rgba(255, 255, 255, 0.95) 100%)',
    keywords: [
      'developer',
      'coding',
      'code',
  ...
  },
  {
    ...
    lightGradient:
      'linear-gradient(135deg, rgba(126, 34, 206, 0.08) 0%, rgba(255, 255, 255, 0.95) 100%)',
  ...
  },
  {
    ...
    lightGradient:
      'linear-gradient(135deg, rgba(180, 83, 9, 0.08) 0%, rgba(255, 255, 255, 0.95) 100%)',
    keywords: [
      'rag',
      'search',
      'conversational',
  ...
  },
  {
    ...
    lightGradient:
      'linear-gradient(135deg, rgba(3, 105, 161, 0.08) 0%, rgba(255, 255, 255, 0.95) 100%)',
    keywords: [
      'cloud',
      'architecture',
      'industrial',
  ...
  },
  {
    ...
    lightGradient:
      'linear-gradient(135deg, rgba(4, 120, 87, 0.08) 0%, rgba(255, 255, 255, 0.95) 100%)',
    keywords: [
      'workplace',
      'productivity',
      'communication',
  ...
  },
  {
    ...
    lightGradient:
      'linear-gradient(135deg, rgba(190, 18, 60, 0.08) 0%, rgba(255, 255, 255, 0.95) 100%)',
  ...
  },
  {
    ...
    lightGradient:
      'linear-gradient(135deg, rgba(21, 128, 61, 0.08) 0%, rgba(255, 255, 255, 0.95) 100%)',
    keywords: [
      'finance',
      'fintech',
      'e-commerce',
  ...
  },
  {
    ...
    lightGradient:
      'linear-gradient(135deg, rgba(190, 24, 93, 0.08) 0%, rgba(255, 255, 255, 0.95) 100%)',
    keywords: [
      'browser',
      'multimedia',
      'social',
 ...
 */
export function parseCategoryLabel(category: string): {
  emoji: string;
  label: string;
} {
  ...
  const match = category.match(
    /^(\p{Extended_Pictographic}(?:‍\p{Extended_Pictographic}|️|\p{Emoji_Modifier})*)\s*/u,
  );
  if (match) {
...
export function getCategoryMeta(category: string) {
  ...
  const matchedGroup =
    CATEGORY_GROUPS.find((g) => g.keywords.some((kw) => lower.includes(kw))) ||
    CATEGORY_GROUPS[0];
...
/**
 * Case- and whitespace-insensitive comparison key for a category string.
 * Collapses runs of whitespace so "☁️  Cloud Platforms" folds onto
 * "☁️ Cloud Platforms".
 */
...
/**
 * Resolves a caller-supplied category (query string, MCP tool argument, agent
 * input) to the exact category string stored on listings, or undefined when the
 * catalog has no such category.
 *
 * Accepts, case-insensitively and with surrounding/duplicated whitespace ignored:
 *   - the full stored name — "🗄️ Databases"
 *   - the plain label      — "Databases"
 *   - the catalog slug     — "databases", "search-and-data-extraction"
 *   - legacy slugs and the CATEGORY_ALIASES dictionary — "dev tools", "sql"
 *
 * Filtering queries compare against the stored category *exactly* (D1
 * `category = ?`), so an unresolved string must never be passed through: doing
 * that is what made `?category=databases` return zero results for every
 * category. Read paths must resolve here first.
 *
 * This is the read-side counterpart to normalizeCategory(), which is for writes
 * and therefore always coerces to some valid category (fuzzy substring matching
 * included) so a submission can never store an unknown one. A search filter must
 * NOT do that — silently answering a different question than the caller asked is
 * worse than saying the category is unknown — so this returns undefined instead.
 */
export function resolveCategoryParam(
  ...
  // 1. Exact stored name.
  if (DIRECTORY_CATEGORIES.includes(raw)) return raw;

  // 2. Stored name, ignoring case/whitespace ("☁️ cloud platforms").
  const nameMatch = DIRECTORY_CATEGORIES.find(
    (c) => foldCategory(c) === folded,
  );
  if (nameMatch) return nameMatch;

  // 3. Plain label without the emoji ("Databases").
  const labelMatch = DIRECTORY_CATEGORIES.find(
    (c) => foldCategory(parseCategoryLabel(c).label) === folded,
  );
  if (labelMatch) return labelMatch;

  // 4. Catalog or legacy slug ("databases", "dev-tools").
  const slugMatch = categoryFromSlug(folded);
  if (slugMatch) return slugMatch;

  // 5. Anything that slugifies onto a catalog slug — covers "&" vs "and"
  //    spellings and stray punctuation ("Search and Data Extraction").
  const slugifiedMatch = categoryFromSlug(categorySlug(raw));
  if (slugifiedMatch) return slugifiedMatch;

  // 6. Explicit alias dictionary ("sql", "coding").
...
export function categoryIntroCopy(
  category: string,
  count: number,
  topNames: string[],
): string {
...
export function normalizeCategory(input?: string | null): string {
  ...
  const labelMatch = DIRECTORY_CATEGORIES.find(
    (c) => parseCategoryLabel(c).label.toLowerCase() === lower,
  );
  if (labelMatch) return labelMatch;
  ...
  for (const canonical of DIRECTORY_CATEGORIES) {
    ...
    if (
      cleanLabel === lower ||
      cleanLabel.includes(lower) ||
      lower.includes(cleanLabel)
    ) {
      return canonical;
    }
  }
