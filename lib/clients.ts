...
export const MCP_CLIENTS: McpClient[] = [
  {
    ...
    steps: [
      { title: 'Open the config file', body: 'In Claude Desktop, go to Settings → Developer → Edit Config. This opens (or creates) claude_desktop_config.json in your text editor. You can also open the file directly at the path for your OS.' },
      { title: 'Add the server under mcpServers', body: 'Add an entry inside the "mcpServers" object with a name, a "command", and its "args". Put any secrets in the "env" object rather than inline in the args.' },
      { title: 'Save and fully restart Claude Desktop', body: 'Quit Claude Desktop completely (not just the window) and reopen it. MCP servers are only launched on startup, so config edits are ignored until a full restart.' },
      { title: 'Verify the tools loaded', body: 'Open a new chat and look for the tools/MCP indicator. If the server shows zero tools or an error, check the MCP log at ~/Library/Logs/Claude/mcp*.log (macOS) or %APPDATA%\\Claude\\logs (Windows).' },
    ],
    faq: [
      { q: 'Where is the Claude Desktop MCP config file?', a: 'On macOS it is ~/Library/Application Support/Claude/claude_desktop_config.json; on Windows it is %APPDATA%\\Claude\\claude_desktop_config.json. The fastest way to open it is Settings → Developer → Edit Config, which creates the file if it does not exist.' },
      { q: 'Why does my MCP server show zero tools in Claude Desktop?', a: 'The server process usually started but crashed before responding to the tools/list request — commonly a missing environment variable, a bad command path, or non-JSON output on stdout. Open the per-server log at ~/Library/Logs/Claude/mcp*.log to see the actual error.' },
      { q: 'Do I need to restart Claude Desktop after editing the config?', a: 'Yes. Claude Desktop only launches MCP servers at startup, so you must fully quit and reopen the app — closing the window is not enough.' },
    ],
  },
  {
    slug: 'cursor',
...
claude mcp list`,
    steps: [
      { title: 'Add the server with the CLI', body: 'Run "claude mcp add <name> -- <command> [args...]". Everything after the -- is the command Claude Code runs to launch the server. Use -e KEY=value to pass environment variables.' },
      { title: 'Choose a scope', body: 'Add --scope project to write the server into a shared .mcp.json your team commits, or --scope user to make it available across all your projects. Local scope (the default) keeps it to the current project for just you.' },
      { title: 'Verify the connection', body: 'Run "claude mcp list" to see each server and whether it connected. You can also use the /mcp command inside a Claude Code session to inspect available servers and tools.' },
    ],
    faq: [
      { q: 'How do I add an MCP server to Claude Code?', a: 'Use the terminal: claude mcp add <name> -- <command> [args...]. For example, claude mcp add github -- npx -y @modelcontextprotocol/server-github. Add -e KEY=value for secrets and --scope project or user to control where it is stored.' },
      { q: 'What is the difference between project and user scope in Claude Code?', a: 'Project scope writes to a .mcp.json in the repository so your whole team shares the server; user scope stores it in your global config so it follows you across projects. Local (default) scope keeps it to the current project for only you.' },
      { q: 'How do I check whether a Claude Code MCP server connected?', a: 'Run claude mcp list from the terminal, or use the /mcp slash command inside a session. For verbose connection logs, start Claude Code with the --debug flag.' },
    ],
  },
  {
    slug: 'vs-code',
    ...
    configLocations: [
      ...
      { os: 'User', path: 'the "mcp" section of your VS Code user settings.json' },
    ],
    configExample: `{
  "servers": {
    "filesystem": {
    ...
    steps: [
      { title: 'Create the MCP config', body: 'Run "MCP: Add Server" from the Command Palette, or create .vscode/mcp.json in your project. Note that VS Code nests servers under a "servers" key — not "mcpServers" like other clients.' },
      { title: 'Add the server definition', body: 'Add a named entry with a "command" and "args". For secrets, use VS Code "inputs" so the value is prompted for and stored securely instead of hard-coded in the file.' },
      { title: 'Enable it in Copilot agent mode', body: 'Open Copilot Chat and switch to Agent mode, then start the server from the MCP prompt. The tools become available for Copilot to call during agent tasks.' },
    ],
    faq: [
      { q: 'How do I add an MCP server to VS Code?', a: 'Create a .vscode/mcp.json in your project (or run "MCP: Add Server" from the Command Palette) and add the server under a "servers" key. MCP tools are used from GitHub Copilot Chat in Agent mode.' },
      { q: 'Why does VS Code use "servers" instead of "mcpServers"?', a: 'VS Code adopted its own schema and nests MCP servers under a "servers" key, with support for a separate "inputs" array for prompted secrets. If you copy a config from Claude or Cursor, rename "mcpServers" to "servers".' },
      { q: 'Do I need Copilot agent mode to use MCP in VS Code?', a: 'Yes — MCP tools are invoked from Copilot Chat in Agent mode. Make sure you have Copilot enabled and select Agent mode before expecting the server tools to be called.' },
    ],
  },
];
